 ...
 * - 优化版本：批次处理优化
 */
public class ReportGeneratorV3 {

    FileFetcher fileFetcher;

    public ReportGeneratorV3(FileFetcher fileFetcher) {
    ...
    private Map<String, InputStream> getFileStreamMap(List<CheckResult> checkResults) {
        ...
        // 2. 批量获取所有图片流封装为Map(多线程版本：多线程调用获取单个截图文件接口（基于文件服务没有提供批量接口的场景下）)
        Map<String, InputStream> fileStreamMap = fileFetcher.fetchFiles(fileInfoList);
    ...
    public void generateReport(List<CheckResult> checkResults, String outputPath) throws Exception {
        ...
        long getFileTime = endTime1 - startTime1;
        System.out.println("截图数据获取处理耗时：" + getFileTime + "ms");
        ...
        long createDocxFileTime = endTime2 - startTime2;
        System.out.println("文档处理耗时：" + createDocxFileTime + "ms");

        System.out.println(String.format("截图数据获取处理耗时：[%d]ms,文稿处理耗时：[%d]ms ", getFileTime, createDocxFileTime));
    }
}
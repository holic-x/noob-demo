...
public class ForeLoginJFrame extends JFrame implements MouseListener,FocusListener{
	//定义全局组件
				...
				if(findUser!=null){
					...
					if(limit==0){
						...
						if(emp!=null){
							...
							if(dept_id.equals("3153961019")||dept_id.equals("1674836044")||dept_id.equals("2249164713")||dept_id.equals("0653000153"))
							{
								JOptionPane.showMessageDialog(null, "用户登录成功，欢迎您，"+emp.getEmployee_name()+"!");
								//隐藏当前的页面，进入到主页面
								this.setVisible(false);
								new ForeIndexJFrame(findUser);
							}else{
								// 处理还没有指定相应员工的账号是无法使用的
								JOptionPane.showMessageDialog(null, "抱歉，当前该员工账号不能操作前台管理模块！");
							}
						}else{
							// 处理还没有指定相应员工的账号是无法使用的

...
public class WarehouseManagerJFrame extends JFrame implements MouseListener,
		FocusListener {
	// 定义全局组件
	 ...
	 * 通过构造方法完成初始化
	 */
	public WarehouseManagerJFrame(StockManagerJPanel parentPanel) {
	...
	private void initTablePanel() {
		...
		// 输入的数据进行查找
		String[] params = { "仓库id", "仓库名称", "仓库标识" };
		Vector<Vector> vec = new Vector<>();
		if (!keyword.getText().equals("请输入仓库关键字")) {
			try {
				// 拼接查找的字符串
				String text = "%" + keyword.getText() + "%";
				vec = warehouseService.getWarehouseByNameKeyword(text);
			} catch (SQLException e) {
				e.printStackTrace();
			}
		} else {
			try {
				vec = warehouseService.findAllWarehouseVector();
			} catch (SQLException e) {
				e.printStackTrace();
			}
		}
	...
	public void mouseClicked(MouseEvent e) {
		...
		if (e.getSource() == tool_add) {
			String name = JOptionPane.showInputDialog(null, "请输入仓库名称");
			// 下述语句用于处理空指针异常
			if (name == null) {
				name = "";
			}
			if (!name.equals("")) {
				int choose = JOptionPane.showConfirmDialog(null, "确定增加这条仓库信息？");
				if (choose == 0) {
					// 随机生成10位的随机字符作为仓库id
					String id = RandomGeneration.getRandom10charSeq();
					Warehouse u = new Warehouse(id, name, 0);
					try {
						warehouseService.addWarehouse(u);
			...
			if (row < 0) {// 没有选中任何收据
				...
				if (!newName.equals("")) {
					int choose = JOptionPane.showConfirmDialog(null,
							"确定修改这条仓库信息？");
					if (choose == 0) {
						// 从数据表中获取当前选中的仓库信息的仓库id
						String id = (String) table.getValueAt(row, 0);
						Warehouse u = new Warehouse(id, newName, 0);
						try {
							warehouseService.updateWarehouse(u);
						} catch (SQLException e1) {
							e1.printStackTrace();
						}
						// 显示修改成功信息面板，并刷新数据面板
			...
			if (row < 0) {// 没有选中任何收据
				...
				// 获取当前选中仓库的id
				String id = (String) table.getValueAt(row, 0);
				int result = JOptionPane.showConfirmDialog(null, "确认删除这条仓库信息？");
				if (result == 0) {
					// 确认删除，则执行删除
					try {

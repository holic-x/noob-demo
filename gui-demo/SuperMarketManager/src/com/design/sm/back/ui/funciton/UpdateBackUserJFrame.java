...
import com.design.sm.back.ui.control.BackIndexJFrame;
 ...
 * 用户修改基本账号信息
 */
public class UpdateBackUserJFrame extends JFrame implements FocusListener,
		MouseListener {
	...
	// 定义父对象、当前登录用户
	BackIndexJFrame parentPanel;
	Accounts loginUser;

	// 通过构造方法初始化数据
	public UpdateBackUserJFrame(BackIndexJFrame parentPanel, Accounts loginUser) {
	...
	public void mouseClicked(MouseEvent e) {
		if (e.getSource() == save_button) {
			...
			if (name_string.equals("")) {
				...
				String confirm_pwd = this.loginUser.getPassword();
				if (!pwd_old_string.equals(confirm_pwd)) {
					...
					try {
						if(userService.isValidAccountsname(name_string)){
							...
							// 用户id、用户权限不可轻易改变
							String id = this.loginUser.getAccount_id();
							int limit = this.loginUser.getLimits();
							Accounts user = new Accounts(id, name_string,
							...
							if (choose == 0) {
								...
								JOptionPane.showMessageDialog(null, "账号信息修改成功!");
	...
	public void echoData() {
		...
		String name_string = this.loginUser.getUsername();
		int limit = this.loginUser.getLimits();

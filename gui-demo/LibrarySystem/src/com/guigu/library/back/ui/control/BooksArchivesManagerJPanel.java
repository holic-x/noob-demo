...
import com.guigu.library.back.ui.function.AddBooksJFrame;
import com.guigu.library.back.ui.function.ShowBooksDetailJFrame;
import com.guigu.library.back.ui.function.UpdateBooksJFrame;
import com.guigu.library.export.ui.BooksExportCheckBoxJFrame;
...
public class BooksArchivesManagerJPanel implements MouseListener,
	...
	// 定义用到的标签
	JLabel label_search, tool_add, tool_modify, tool_delete, tool_put_on,
	 ...
	 * 通过构造方法完成初始化
	 */
	public BooksArchivesManagerJPanel(Users user) {
	...
	private void initToolPanel() {
		...
		tool_add.setToolTipText("插入图书信息");// 设置鼠标移动时的显示内容
		tool_add.addMouseListener(this);// 添加鼠标监听

		Icon icon_modify = new ImageIcon("icons/toolImage/modify.png");
		tool_modify = new JLabel(icon_modify);
		tool_modify.setToolTipText("修改图书信息");// 设置鼠标移动时的显示内容
		tool_modify.addMouseListener(this);// 添加鼠标监听

		Icon icon_delete = new ImageIcon("icons/toolImage/delete.png");
		tool_delete = new JLabel(icon_delete);
		tool_delete.setToolTipText("删除图书信息");// 设置鼠标移动时的显示内容
		tool_delete.addMouseListener(this);// 添加鼠标监听

		Icon icon_put_on = new ImageIcon("icons/toolImage/put_on.png");
		tool_put_on = new JLabel(icon_put_on);
		tool_put_on.setToolTipText("图书上架/下架");// 设置鼠标移动时的显示内容
		...
		tool_detail.setToolTipText("查看图书详情");// 设置鼠标移动时的显示内容
		...
		tool_import.setToolTipText("导入图书信息");// 设置鼠标移动时的显示内容
		...
		tool_export.setToolTipText("导出图书信息");// 设置鼠标移动时的显示内容
	...
	public void mouseClicked(MouseEvent e) {
		...
		if (e.getSource() == label_all) {
			...
			if (row < 0) {// 没有选中任何收据
				...
				// 修改图书信息
				new UpdateBooksJFrame(this, table, row);
			}
		} else if (e.getSource() == tool_delete) {
			// 删除当前选中的数据
			int row = table.getSelectedRow();// 得到选中的行
			if (row < 0) {// 没有选中任何收据
				JOptionPane.showMessageDialog(null, "请选择要删除的图书信息");
			} else {
				// 获取当前选中图书id
				String book_id = (String) table.getValueAt(row, 0);
				int result = JOptionPane.showConfirmDialog(null, "确认删除这条图书信息？");
				if (result == 0) {
					// 确认删除，则执行删除
					try {
						booksService.deleteBooks(book_id);
		...
		} else if (e.getSource() == tool_put_on) {
			try {
				// 图书上架、获取当前选中的行信息
				int row = table.getSelectedRow();
				if (row < 0) {
					JOptionPane.showMessageDialog(null, "请选择要进行上下架操作的图书信息！");
				} else {
					// 如果是已上架的书籍则询问是否进行下架处理
					int put_on_flag = Integer.valueOf(table.getValueAt(row, 21)
							.toString());
					if (put_on_flag == 1) {
						int choose = JOptionPane.showConfirmDialog(null,
								"当前书籍已上架，请选择是否要进行下架操作？");
						if (choose == 0) {
							String book_id = table.getValueAt(row, 0)
									.toString();
							Books book = booksService.getBooksById(book_id);
							if (book != null) {
								book.setPut_on_flag(0);
								booksService.updateBooks(book);
								JOptionPane.showMessageDialog(null, "书籍下架成功！");
								this.refreshTablePanel();
							}
						}
					} else if (put_on_flag == 0) {
						// 如果是已下架的书籍则询问是否进行上架处理
						int choose = JOptionPane.showConfirmDialog(null,
								"当前书籍处于下架状态，请选择是否要进上架操作？");
						if (choose == 0) {
							String book_id = table.getValueAt(row, 0)
									.toString();
							Books book = booksService.getBooksById(book_id);
							if (book != null) {
								book.setPut_on_flag(1);
								booksService.updateBooks(book);
								JOptionPane.showMessageDialog(null, "书籍上架成功！");
								this.refreshTablePanel();
							}
						}
					}
			...
			int row = table.getSelectedRow();// 得到选中的行
			if (row < 0) {// 没有选中任何收据
				JOptionPane.showMessageDialog(null, "请选择要查阅的图书信息");
			} else {
				// 查看图书详细信息
				new ShowBooksDetailJFrame(this, table, row);
			}
		} else if (e.getSource() == tool_import) {
			// 导入图书数据
			// 待定
		} else if (e.getSource() == tool_export) {
			// 导出图书数据（根据选中的行、选中的列导出数据信息）
			// 获取当前选中的数据
			int[] selectedRows = table.getSelectedRows();
			new BooksExportCheckBoxJFrame(this, table, selectedRows);
		}
	}

...
import com.guigu.library.fore.ui.function.ShowBooksDetailJFrameByReader;
...
public class BooksSearchJPanel implements MouseListener, FocusListener,
		ItemListener {
	...
	// 定义用到的标签
	JLabel label_search,tool_detail,tool_save,tool_favorite;
	 ...
	 * 通过构造方法完成初始化
	 */
	public BooksSearchJPanel(Users user) {
		...
		page.getDocument().addDocumentListener(new DocumentListener() {
			...
			public void insertUpdate(DocumentEvent e) {
				...
				if (pattern.matcher(page.getText()).matches()) {
					if(Integer.valueOf(page.getText())>getMaxPage(table.getRowCount())){
						JOptionPane.showMessageDialog(null, "输入页数不能超过当前最大页数！");
					}else {
	...
	public void mouseClicked(MouseEvent e) {
		...
		if (e.getSource() == label_all) {
			...
			if(row<0){
				...
				new ShowBooksDetailJFrameByReader(this,table,row);
			}
			...
			if(row<0){
				...
				try {
					...
					// 根据当前登录用户获取Reader信息
					Reader r = readerService.getReaderByUserId(this.user.getUser_id());
					if(r!=null){
						reader_id = r.getReader_id();
					}
					Temp t = tempService.getTempByISBN(b.getIsbn(),reader_id);
					if(t!=null){
						JOptionPane.showMessageDialog(null, "该书籍已添加至收藏夹，不能重复操作！");
					}else{

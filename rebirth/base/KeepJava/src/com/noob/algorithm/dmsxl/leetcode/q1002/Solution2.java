...
public class Solution2 {
    ...
    public List<String> commonChars(String[] words) {
        ...
        // 以words[0]为基础，校验其他字符串中是否出现该字符
        Set<Character> keySet = map[0].keySet();
        for (char cur : keySet) {
            int curCnt = map[0].get(cur);
            while (curCnt-- > 0) { // 对于多次重复出现的字符，需根据出现此处进行处理
                // 校验其他字符串是否出现该字符
                boolean allMatch = true;
                for (int i = 0; i < len; i++) {
                    boolean match = false;
                    for (Map.Entry<Character, Integer> entry : map[i].entrySet()) {
                        if (entry.getKey() == cur && entry.getValue() > 0) {
                            match = true; // 当前匹配
                            entry.setValue(entry.getValue() - 1); // 使用次数-1
                            // curCnt--;
                            break;
                        }
                    }

...
TEST_F(Exp_01_04_UpdateTest, Correct){
   ...
   // EXPECT_EQ(0, update("update student set sname ='test'"));
...
//    EXPECT_EQ(_expect_list[9], update(_m_list[9]));
    ...
    // 无指定where表达式update操作测试
   // EXPECT_EQ(9, update("update student set sage=0"));
//    EXPECT_EQ(9, select("select * from student where sage=0"));

    EXPECT_EQ(9, update("update set sage=0"));
    EXPECT_EQ(9, update("update student sage=0"));
    EXPECT_EQ(9, update("update student set sage"));
    EXPECT_EQ(9, update("update student set sage="));
    EXPECT_EQ(9, update("update student set sage='error test'"));
